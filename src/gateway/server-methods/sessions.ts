...
export const sessionsHandlers: GatewayRequestHandlers = {
  ...
  "sessions.reset": async ({ params, respond }) => {
    ...
    const next = await updateSessionStore(storePath, (store) => {
      ...
      // Delete old transcript file(s) so the LLM gets zero context on the next run.
      if (entry?.sessionId) {
        const pathsToRemove: string[] = [];
        if (entry.sessionFile?.trim()) {
          pathsToRemove.push(entry.sessionFile.trim());
        }
        pathsToRemove.push(resolveSessionTranscriptPath(entry.sessionId, target.agentId));
