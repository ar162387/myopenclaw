...
import { promptSensorTowerSetup } from "./sensortower-setup.js";
...
async function promptSensorTowerToolsConfig(
  nextConfig: OpenClawConfig,
  runtime: RuntimeEnv,
): Promise<OpenClawConfig> {
  return await promptSensorTowerSetup({
    cfg: nextConfig,
    io: {
      ...
      text: async (params) => guardCancel(await text(params), runtime),
    },
    initialEnable: false,
  });
}
...
export async function runConfigureWizard(
  ...
  try {
    ...
    if (opts.sections) {
      ...
      if (selected.includes("sensortower")) {
        nextConfig = await promptSensorTowerToolsConfig(nextConfig, runtime);
      }
