...
import { loadSessions, resetSession } from "./controllers/sessions";
...
type NewSessionHost = ChatHost & { lastError: string | null };

export async function handleNewSession(host: NewSessionHost): Promise<void> {
  if (!host.connected) {
  ...
  host.lastError = null;
  try {
    await resetSession(host as unknown as Parameters<typeof resetSession>[0], host.sessionKey);
  } catch (err) {
  ...
  await sendChatMessageNow(host, "/new", {
    ...
    previousAttachments: host.chatAttachments,
